...
w0=2*pi/T;
x=1/4; %masukkan koefisien a0 di sini
for n=1:nmax
a = cos(n*w0*t)*((-2/(n*pi)^2)*cos(n*(pi/2))+4/(n*pi)^2-(2/(n*pi)^2)*cos(n*(3/2)*pi));
...
ylabel('Amplitude')
grid on
axis([-7 7 -0.5 1.5]) % dapat diubah - ubah untuk menampilkan gambar dgn
% lebih baik
C0 = 1/4;
...
for n=1:coef_nmax
an = (-2/(n*pi)^2)*cos(n*(pi/2))+4/(n*pi)^2-(2/(n*pi)^2)*cos(n*(3/2)*pi);
