...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
t=-9:0.01:9; % time vector, rentang observasi isyarat
nmax=100; % banyaknya iterasi, dapat diubah - ubah sesuai permintaan
T=4; %periode isyarat yang diamati
w0=2*pi/T;
x=-1; %masukkan koefisien a0 di sini
for n=1:nmax
a = cos(n*w0*t)*((4/(n*pi)^2)*cos(n*pi/2)-4/(n*pi)^2-(4/3)*(1/(n*pi)^2)+(4/3)*(1/(n*pi)^2)*cos(n*pi/2));
%masukkan koefisien an di sini lalu kalikan dengan cos(n*w0*t)
b = sin(n*w0*t)*((4/(n*pi)^2)*sin(n*pi/2)+(4/3)*(1/(n*pi)^2)*sin(n*pi/2));
...
ylabel('Amplitude')
grid on
axis([-7 9 -3 1]) % dapat diubah - ubah untuk menampilkan gambar dgn
% lebih baik
C0 = -1;
...
for n=1:coef_nmax
an = (4/(n*pi)^2)*cos(n*pi/2)-4/(n*pi)^2-(4/3*(n*pi)^2)+(4/3*(n*pi)^2)*cos(n*pi/2);
%masukkan koefisien an di sini
bn = (4/(n*pi)^2)*sin(n*pi/2)+(4/3*(n*pi)^2)*sin(n*pi/2); 
%masukkan koefisien bn di sini
an=0.5.*an;
bn=0.5.*bn;
